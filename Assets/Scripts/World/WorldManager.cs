...
namespace World
{
    ...
    public class WorldManager : SingletonComponent<WorldManager>
    {
        ...
        private void Start()
        {
            ...
            m_cache = new Cache($"{cachePath}/{cacheFileName}.vox");
        ...
        {
            foreach (KeyValuePair<int3, Chunk> keyValuePair in m_chunks)
            {
                if (keyValuePair.Value.IsDirty())
                {
                    m_cache.Write(keyValuePair.Value, keyValuePair.Key);
                }
            }
            m_voxelConfig.VoxelVolumeConfig.OnLateDirtied -= ApplyVoxelVolumeConfig;
            m_voxelConfig.DualContouringConfig.OnLateDirtied -= ApplyDualContouringConfig;
        ...
        public static Dictionary<int3, Chunk> GetActiveChunks()
        {
            return Instance.m_chunks;
        }
        ...
        private void DestroyChunksOutsideViewDistance(float3 viewerPosition)
        {
            ...
            {
                if (m_chunks[chunkCoordinate].IsDirty())
                {
                    m_cache.Write(m_chunks[chunkCoordinate], chunkCoordinate);
                }
        ...
        private void CreateChunksWithinViewDistance(float3 viewerPosition)
        {
            ...
            while (m_chunksToProcess.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (!m_cache.ReadIfExist(chunk, chunkCoordinate))
                    {
